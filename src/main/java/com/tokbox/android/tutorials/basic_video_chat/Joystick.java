...
public class Joystick extends FrameLayout {
    ...
    private boolean dragExceedsSlop(MotionEvent event) {
        ...
        final float dx = abs(x - downX);
        final float dy = abs(y - downY);
...
/*        switch (motionConstraint) {
        ...
        Xcoord = x;
        Ycoord = y;
        //limit values to + or - 128
        if (Xcoord > 128) Xcoord = 128;
        if (Xcoord < -128) Xcoord = -128;

        if (Ycoord > 128) Ycoord = 128;
        if (Ycoord < -128) Ycoord = -128;
        Ycoord = Ycoord*-1; //invert sign

        Log.d(LOG_TAG, "Xcoord" + (Xcoord));
        Log.d(LOG_TAG, "Ycoord" + (Ycoord));
